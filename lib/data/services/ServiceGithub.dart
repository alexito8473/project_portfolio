...
class ServiceGithub {
  ...
  Future<void> seeCommitGithub() async {
    int countCommits = 0;

    final userResponse = await http
        .get(Uri.parse('https://api.github.com/users/$userName'), headers: {
    ...
    if (userResponse.statusCode == 200) {
      ...
      final reposResponse = await http.get(
          ...
          headers: {'Authorization': 'token $tokenGithub'});
        ...
        for (var repo in reposData) {
          ...
          if (commitsResponse.statusCode == 200 ) {
            ...
            for (var contributor in contributorsData) {
              countCommits +=
                  int.parse(contributor["total"].toString());
            }
